...
Science, acotadas al período 2022--2026. La elección de estos
estudios se hizo en cuatro pasos donde se aplicó criterios de
inclusión y exclusión predefinidos con anterioridad.
\textbf{Resultados:} De 1\,107 registros recuperados, se retuvieron
35 estudios primarios. Se notó que los enfoques más frecuentes son las
arquitecturas de detección basadas en fusión multiespectral
(RGB + infrarrojo o térmico), las redes con módulos de
atención y las variantes compactas y optimizadas que tiene YOLO, para trabajar en tiempo
real en el área existente. Las métricas predominantes son la precisión media
(\textit{mAP}), el porcentaje de la tasa de error de detección y los fotogramas
por segundo (\textit{FPS}).
\textbf{Conclusiones:} Las estrategias con más amparo científico son la fusión multimodal y los mecanismos de atención que son usados
para gestionar la oclusión y situaciones de baja iluminación; sin embargo, aún
existen carencias en la generalización cuando se trata sobre escenarios reales y en
la disponibilidad de grupos de datos que combinen estas dos condiciones
adversas simultáneamente en el mismo área de desenvolvimiento.
...
studies was carried out in four steps where previously predefined inclusion
and exclusion criteria were applied.
\textbf{Results:} From 1,107 retrieved records, 35 primary studies
were retained. It was noted that the most frequent approaches are detection
architectures based on multispectral fusion (RGB + infrared or thermal),
networks with attention modules, and the compact and optimized variants that YOLO has,
to work in real time within the existing area. The predominant metrics are mean average precision
(\textit{mAP}), the percentage of the detection miss rate, and frames per
second (\textit{FPS}).
\textbf{Conclusions:} The strategies with the most scientific support are multimodal fusion and attention mechanisms that are used
to manage occlusion and low illumination situations; however, shortcomings
still exist in generalization when dealing with real-world scenarios and in
the availability of datasets that combine these two adverse conditions
simultaneously in the same development area.
...
La comunidad academica de la ciencia ha explorado diversas respuestas ante esta
problem\'{a}tica existente: la integracion de canales espectrales
adicionales ---im\'{a}genes con tecnologia infrarroja  o t\'{e}rmicas--- para disminuir
la dependencia de la luz visible~\cite{E35, E31, E19, E26}; el
dise\~{n}o de m\'{o}dulos de atenci\'{o}n que primen las regiones
parcialmente visibles~\cite{E25, E32, E23}; y el desarrollo de las
arquitecturas compactas orientadas a la inferencia en tiempo real sobre el
hardware con recursos limitados~\cite{E21, E23}. Sin embargo, tdoavia no
existe a la fecha actual una s\'{i}ntesis sustancialmente estructurada de estos avances
espec\'{i}ficamente apuntada al contexto de videovigilancia con
oclusi\'{o}n y condiciones de vaja visibilidad que trabajen de forma simult\'{a}nea.
...
Analizar estos 35 estudios primarios permite agrupar los principales
modelos identificados en cuatro familias. Las
\textbf{arquitecturas basadas en YOLO} (YOLOv8s, NP-YOLO, PYOLO)
representan el enfoque de una sola etapa m\'{a}s frecuente, valorado esencialmente
por su capacidad de procesar m\'{u}ltiples fotogramas por cada segundo con
recursos computacionales de hardware moderado~\cite{E21,E18,E12,E23}. Los
\textbf{detectores de fusi\'{o}n multiespectral} combinan las im\'{a}genes
del espectro visible con canales de tecnologia infrarroja o t\'{e}rmica, aportando
robustez ante condiciones nocturnas y de niebla densa; esto ofrece
informaci\'{o}n adicional independiente de la iluminaci\'{o}n
ambiental del area~\cite{E35,E31,E19,E17,E07,E04,E26}. Las \textbf{redes con
...
informativas, aumentando la sensibilidad ante figuras parcialmente
encubiertas~\cite{E25, E05, E14}. Finalmente, los \textbf{enfoques
multi-vista} se benefician del uso de varias c\'{a}maras simult\'{a}neas
para resolver ambig\"{u}edades que son originadas por la oclusi\'{o}n mutua
entre personas~\cite{E09, E30, E10}.

\begin{hallazgo}[]
  Los modelos de fusi\'{o}n multiespectral (RGB + IR/t\'{e}rmico) y las
  versiones que se hallan de YOLO constituyen los enfoques con una
  presencia significativamente mayor en la literatura revisada, identificados en el 63\,\% de
  los estudios primarios. La fusi\'{o}n multimodal es la estrategia principalmente
  predominante para obtener los mejores resultados ante condiciones de  baja visibilidad
  en horas de la noche, a la vez que los m\'{o}dulos de atenci\'{o}n son la
  respuesta de mayor eleccion ante la oclusi\'{o}n
...
Los estudios primarios encontrados documentan estas cinco estrategias complementarias
para el manejo de la oclusi\'{o}n:
\begin{enumerate}
  \item \textbf{Predicci\'{o}n de partes anatomicas corporales visibles ante la camara:} el
        detector genera cuadros parciales sobre las partes que estan expuestas ante la camara
        (cabeza, abdomen,muslos) y supone la posici\'{o}n global a partir de
        ellas~\cite{E27}.
  \item \textbf{Estimaci\'{o}n de la posicion vertical mediante esqueletos:} la
        reconstrucci\'{o}n del esqueleto aun visible permite congeturar de manera eficar la postura incluso cuando parte del cuerpo
        est\'{a} porcentualmente ocluida~\cite{E27}.
  \item \textbf{Supresi\'{o}n de no m\'{a}ximos adaptada (NMS):}
        son variantes que an sido predise\~{n}adas para conservar las detecciones de peatones
        parcialmente tapados en escenas densas que dificultan una vision ideal~\cite{E29, E28, E11}.
...
Los hallazgos tras haver realizado esta revisi\'{o}n son 
acordes a la tendencia general documentada en los trabajos previos
sobre detecci\'{o}n peatonal~\cite{Brunetti2018, Dollar2012}: la
calidad del detector depende de forma critica de la riqueza y la
disponibilidad de informaci\'{o}n visual. En el momento en que la iluminaci\'{o}n o
la visibilidad se ven comprometidas, la incorporaci\'{o}n de canales
espectrales adicionales aparece como la estrategia con mayor efectividad
...
variantes de YOLO refleja la presi\'{o}n industrial por soluciones que tengan
la capacidad de operar en tiempo real con hardware eficiente y que sea factible acceso.
...
adversas sin que altere la velocidad de inferencia, lo que los hace
...
Los cinco vacios de conocimiento identificadas en la RQ6 configuran
una agenda de investigaci\'{o}n clara. La de mayor urgencia es la
falta considerable de benchmarks que combinen simult\'{a}neamente
oclusi\'{o}n densa y condiciones meteorol\'{o}gicas de dificil identificacion. Los
conjuntos existentes (Caltech, KAIST, FLIR) tocan estas dimensiones
de forma separada, lo que impide la validaci\'{o}n cruzada de modelos
que pretendan ser robustos ante estas condiciones en conjunto. La segunda brecha,
vinculada a la adaptaci\'{o}n de dominio, cobra especial relevancia en
el contexto latinoamericano es cuando las condiciones de infraestructura
...
\begin{itemize}
  \item \textbf{Validez de la construcci\'{o}n:} la cadena de
        b\'{u}squeda tiene la latente posiblidad existente de no recuperar los estudios que usen terminolog\'{i}a
        alternativa para describir oclusi\'{o}n o baja visibilidad en las camaras usadas. Esta
        amenaza se disminuy\'{o} a traves de la inclusi\'{o}n de
        sin\'{o}nimos y la validaci\'{o}n iterativa de la cadena contra
        un conjunto de art\'{i}culos conocidos.
  \item \textbf{Validez interna:} la selecci\'{o}n y an\'{a}lisis de la
        calidad de los estudios an sido realizadas en conjunto por el equipo
        investigador, lo que introduce un riesgo de sesgo de
        inclusi\'{o}n. Se mitig\'{o} a traves de la adecuada aplicaci\'{o}n
        sistem\'{a}tica de los criterios definidos en el protocolo establecido.
  \item \textbf{Validez externa:} los resultados se limitan al
        per\'{i}odo de tiempo 2022--2026 y a las cuatro bases de datos
  ...
  \item \textbf{Validez de conclusi\'{o}n:} la heterogeneidad de
        m\'{e}tricas y grupos de datos que fueron reportados dificulta de forma muy significativa la
        comparaci\'{o}n quantitativa directa entre los modelos existentes, por lo que
        la s\'{i}ntesis del tema se realiz\'{o} de forma narrativa.
\end{itemize}
...
Esta Revisi\'{o}n Sistem\'{a}tica de Literatura sintetiz\'{o} de forma eficas la
...
Respecto a los modelos que existen actualmente (RQ1), la fusi\'{o}n multiespectral
RGB-infrarrojo/\allowbreak t\'{e}rmico y las redes con m\'{o}dulos de atenci\'{o}n
constituyen los enfoques notablemente mas dominantes, seguidos por variantes compactadas pero utiles de
...
indican las mejoras m\'{a}s consistentes. Las m\'{e}tricas con mayor
uso (RQ3) son mAP, tasa de error logar\'{i}tmica y FPS, mientras
